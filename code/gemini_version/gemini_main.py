"""
Main orchestration pipeline for Google Gemini Multi-Modal Evidence Review.
...
from gemini_version.gemini_analyzer import (
...
def process_claims(
    ...
    print(f"\n{'='*60}")
    print(f"Gemini Evidence Review Agent (1 Request / Min Limit)")
    print(f"{'='*60}")
    ...
    # Resume capability for Gemini output
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run Gemini Evidence Review")
    parser.add_argument("--mode", choices=["test", "sample"], default="test", help="Which dataset to process")
    args = parser.parse_args()

    if args.mode == "sample":
        input_csv = SAMPLE_CLAIMS_CSV
        output_csv = DATASET_DIR / "sample_gemini_output.csv"
    else:
        input_csv = CLAIMS_CSV
        output_csv = DATASET_DIR / "gemini_output.csv"
