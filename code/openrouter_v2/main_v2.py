"""
Main orchestration pipeline V2 for OpenRouter Multi-Modal Evidence Review.
Uses improved prompts_v2 and postprocessor_v2.
...
from openrouter_v2.analyzer_v2 import (
    process_claim_strategy_a,
    get_token_usage,
)
from openrouter_v2.postprocessor_v2 import postprocess_result, format_output_row
...
def process_claims(
    ...
    print(f"\n{'='*60}")
    print(f"OpenRouter V2 Evidence Review (Improved Prompts + PostProc)")
    print(f"{'='*60}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run OpenRouter V2 Evidence Review")
    parser.add_argument("--mode", choices=["test", "sample"], default="test", help="Which dataset to process")
    args = parser.parse_args()

    if args.mode == "sample":
        input_csv = SAMPLE_CLAIMS_CSV
        output_csv = DATASET_DIR / "sample_v2_output.csv"
    else:
        input_csv = CLAIMS_CSV
        output_csv = DATASET_DIR / "output2.csv"
